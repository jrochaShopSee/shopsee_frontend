...
const MediaUploadSection: React.FC<MediaUploadSectionProps> = ({ watch, setValue, isShopifyProduct = false }) => {
    ...
    const MediaField = ({ title, description, dimensions, fieldName, imageUrl, disabled = false }: { title: string; description: string; dimensions: string; fieldName: "productImage" | "productIcon" | "productHeaderImage"; imageUrl?: string; disabled?: boolean }) => (
                ...
                {imageUrl ? (
                    <div className="relative">
                        <div className="flex items-center justify-center">
                            <img src={imageUrl} alt={`${title} preview`} className={`rounded-lg ${fieldName === "productImage" ? "max-h-48" : fieldName === "productIcon" ? "max-h-24" : "max-h-20"}`} />
                        </div>
                        {!disabled && (
                            <div className="absolute top-2 right-2 flex space-x-1">
                                <button type="button" onClick={() => setActiveModal(fieldName)} className="bg-blue-500 text-white rounded-full p-1 hover:bg-blue-600 transition-colors" title="Change image">
                                    <Edit className="h-3 w-3" />
                                </button>
                                <button type="button" onClick={() => removeImage(fieldName)} className="bg-red-500 text-white rounded-full p-1 hover:bg-red-600 transition-colors" title="Remove image">
                                    <X className="h-3 w-3" />
                                </button>
                            </div>
                        )}
        ...
        <>
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                ...
                <div className="p-6 space-y-6">
                    {/* Product Image */}
                    <MediaField title="Product Image" description="This image will be displayed as the main product image" dimensions="360x600px" fieldName="productImage" imageUrl={productImage} />

                    {/* Product Icon */}
                    <MediaField title="Product Icon" description="Small icon representation of your product" dimensions="100x100px" fieldName="productIcon" imageUrl={productIcon} />

                    {/* Header Image - Only show if header type is selected */}
                    {showHeaderImage && <MediaField title="Header Image" description="Header image for your selected header type" dimensions="360x80px" fieldName="productHeaderImage" imageUrl={productHeaderImage} disabled={isShopifyProduct} />}
                </div>
            </div>

            {/* Media Selection Modals */}
            <MediaSelectionModal isOpen={activeModal === "productImage"} onClose={() => setActiveModal(null)} onSelect={(mediaUrl) => handleMediaSelect(mediaUrl, "productImage")} title="Select Product Image" selectedUrl={productImage} />

            <MediaSelectionModal isOpen={activeModal === "productIcon"} onClose={() => setActiveModal(null)} onSelect={(mediaUrl) => handleMediaSelect(mediaUrl, "productIcon")} title="Select Product Icon" selectedUrl={productIcon} />

            <MediaSelectionModal isOpen={activeModal === "productHeaderImage"} onClose={() => setActiveModal(null)} onSelect={(mediaUrl) => handleMediaSelect(mediaUrl, "productHeaderImage")} title="Select Header Image" selectedUrl={productHeaderImage} />
        </>
    );
};
