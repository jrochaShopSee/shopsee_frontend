...
export default function AddCurrencyPage() {
    ...
    const onSubmit = async (data: CurrencyFormData) => {
        ...
        try {
            const result = await settingsApi.createCurrency(data);
            toast.success(result.message || "Currency created successfully");
            router.push("/cms/admin/settings/currency");
        } catch (error: unknown) {
            const errorMessage = error && typeof error === 'object' && 'response' in error
                ? (error.response as { data?: { message?: string } })?.data?.message
                : undefined;
            toast.error(errorMessage || "Failed to create currency");
        } finally {
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-600 mt-1">
                                    Create a new currency for your platform
                                </p>
                            </div>
                        </div>
                    </div>
